...
public class GitLabAuthenticationService implements AuthenticationService {
     ...
     */
    GitLabAuthenticationService(final Config config, final OidcConfiguration oidcConfiguration, final String idToken,
     ...
     * <p>
     * If an ID token is provided, Alpine will validate it and source configured
     * claims from it.
     * <p>
     * If an access token is provided, Alpine will call the IdP's {@code /userinfo}
     * endpoint with it
     * to verify its validity, and source configured claims from the response.
     * <p>
     * If both access token and ID token are provided, the ID token takes
     * precedence.
     * When all configured claims are found in the ID token, {@code /userinfo} won't
     * be requested.
     * When not all claims were found in the ID token, {@code /userinfo} will be
        ...
        try (final var qm = new AlpineQueryManager()) {
            ...
            if (user != null) {
                ...
                if (user.getSubjectIdentifier() == null) {
                    LOGGER.debug(
                            "Assigning subject identifier " + profile.getSubject() + " to user " + user.getUsername());
                    ...
                    LOGGER.error("Refusing to authenticate user " + user.getUsername()
                            + ": subject identifier has changed (" +
                ...
                if (!Objects.equals(user.getEmail(), profile.getEmail())) {
                    LOGGER.debug("Updating email of user " + user.getUsername() + ": " + user.getEmail() + " -> "
                            + profile.getEmail());
                    user.setEmail(profile.getEmail());
                ...
                LOGGER.debug("The user (" + profile.getUsername()
                        + ") authenticated successfully but the account has not been provisioned");
                return autoProvision(qm, profile);
            } else {
                LOGGER.debug(
                        "The user (" + profile.getUsername() + ") is unmapped and user provisioning is not enabled");
    ...
    private void syncGitlabGroups(final AlpineQueryManager qm, final OidcUser user, final List<String> groupNames,
            final String accessToken) {
